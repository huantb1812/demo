...
function HumanMap(container) {
    ...
    human.default_option = {
        ...
        onClick: function (e) {
            ...
            //hidden all item 
            document.querySelectorAll(human.statelist_selector + ' .item').forEach(option => {
                option.classList.add('visibility');
            });
            document.querySelectorAll(human.statelist_selector + ' .item.' + e.key.toLowerCase()).forEach(option => {
                ...
                option.addEventListener('click', () => {
                    ...
                    setTimeout(() => {
                        ...
                        let key = option.querySelector('input').value;
                        human.showMenuChild(key);
                    }, 100);
                });
            });

            document.querySelectorAll(human.conditions_selector + ' .item').forEach(option => {
                option.classList.add('visibility');
            });
        },
    };
    ...
    human.showMenuChild = function (event) {
        document.querySelectorAll(human.conditions_selector + ' .item').forEach(item => {
            item.classList.add('visibility');
        });

        document.querySelectorAll(human.conditions_selector + ' .item.' + event.toLowerCase()).forEach(item => {
...
(function () {
     ...
     */
    HumanMapComponent.prototype.render_statelist = function () {
        ...
        for (let k = 0; k < arrs.length; k++) {
            ...
            for (let i = 0; i < human[key].length; i++) {
                let t = this.renderTemplate('input', {
                    input: {
                        type: 'input',
                        attr: {
                            type: 'radio',
                ...
                statelist += '<label class="item ' + key + ' visibility" >' + t + human[key][i] + '</label>'
            }
        }
        return statelist;
    };

    HumanMapComponent.prototype.render_conditions = function () {
        let conditions = ''
                ...
                let t = this.renderTemplate('input', {
                    input: {
                        ...
                        attr: {
                            ...
                            group_data: key,
                            value: human.conditionsItems[key][i],
                        }
                    }
                });
                conditions += '<label class="item ' + key + ' visibility">' + t + human.conditionsItems[key][i] + '</label>'
            }
        }
        return conditions;
    }
    ...
    HumanMapComponent.prototype.render = function (element) {
        ...
        let statelist = this.render_statelist();
        let conditions = this.render_conditions();
    ...
    HumanMapComponent.prototype.getValue = function () {
        ...
        this.refs.input.forEach(item => {
            if (item.name === 'state_value' && item.checked === true) {
                result.statelist = item.value;
            } else if (item.name === 'condition_value' && item.checked === true) {
                result.conditions = item.value;
            }
        });
