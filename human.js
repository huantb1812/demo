    ...
    onClick: function (e) {
        ...
        human[e.key.toLowerCase()].forEach(item => {
            ...
            option.addEventListener('click', () => {
                let currentActived = document.querySelector("#statelist .item.actived");
                if (currentActived) {
            ...
            document.querySelector("#statelist").append(option)
        })
...
function showMenuChild(event) {
    if (conditionsItems[event.toLowerCase()] && conditionsItems[event.toLowerCase()].length > 0) {
        document.querySelector("#conditions").classList.remove('no-record');

        conditionsItems[event.toLowerCase()].forEach(item => {
            let option = document.createElement("div");
            option.innerText = item;
            option.classList = ['item'];
            option.addEventListener('click', () => {
                let currentActived = document.querySelector("#conditions .item.actived");
                if (currentActived) {
                    currentActived.classList.remove('actived');
                }
                setTimeout(() => {
            ...
            document.querySelector("#conditions").append(option)
        })
    } else {
        document.querySelector("#conditions").innerHTML = 'No data';
        document.querySelector("#conditions").classList.add('no-record');
    }
}

$('.map').mapster(default_option);